...
if (isset($_GET['id'])) {
    ...
    $request_item = $conn->prepare("SELECT * FROM request_items WHERE id = :id");
    $request_item->bindParam(':id', $id);
    $request_item->execute();
    $request_item = $request_item->fetchAll(PDO::FETCH_ASSOC)[0];

    try {
        $stmt = $conn->prepare("UPDATE request_items SET is_fullfilled = 1 WHERE id = :id");
        ...
        $quantity = floatval($request_item['request_quantity']) + floatval($item['quantity']);
